...
function init() {
	...
	// fetch recipes from the JSON API
...
function getRecipesFromStorage() {
	...
	const storedData = localStorage.getItem("recipes");		// read recipes from localStorage
	if (!storedData || storedData === "undefined") {
		return [];
	}
	const dataArray = JSON.parse(storedData) || [];			// string -> array
...
function saveRecipesToStorage(recipes) {
	...
	//            header. It is possible in only a single line, but should
	//            be no more than a few lines.
	const recipeString = JSON.stringify(recipes);		// convert array -> string
	localStorage.setItem("recipes", recipeString);		// save that string to localStorage
}
